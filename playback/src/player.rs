...
use metadata::{AudioItem, FileFormat, Metadata, Track, Album, Artist};
...
impl PlayerInternal {
    ...
    fn load_track(&self, spotify_id: SpotifyId, position: i64) -> Option<(Decoder, f32)> {
        ...
        if let Some(path) = self.metadata_pipe.clone() {
            ...
            let metadata_xml = format!("{}{}{}", title_xml, album_name_xml, artists_xml);
